...
%death function for class FEEDER
%agt=feeder object
%cn - current agent number
%klld=1 if agent dies, =0 otherwise

%feeders die if their food level reaches zero or they are older than max_age
...
klld=0;
thold_food=PARAM.FE_MINFOOD;  %threshold minimum food value for death to occur
thold_age=PARAM.FE_MAXAGE;    %threshold maximum age value for death to occur
cfood=agt.food;               %get current agent food level
age=agt.age;                  %get current agent age

if cfood<=thold_food || age>thold_age  %if food level < threshold and age > max age then agent dies
    IT_STATS.fe_died(N_IT+1)=IT_STATS.fe_died(N_IT+1)+1;  %update statistics
