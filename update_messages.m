...
for cn=1:temp_n
    ...
    if dead==0                          %if agent is not dead
        ...
        MESSAGES.pos(cn,:)=pos;
         if isa(agent{cn},'feeder')
             MESSAGES.atype(cn)=1;
             IT_STATS.fe_tot(N_IT+1)=IT_STATS.fe_tot(N_IT+1)+1;
         elseif isa(agent{cn},'forager')
             MESSAGES.atype(cn)=2;
             IT_STATS.fo_tot(N_IT+1)=IT_STATS.fo_tot(N_IT+1)+1;
         elseif isa(agent{cn},'queen')
             MESSAGES.atype(cn)=3;
             IT_STATS.qu_tot(N_IT+1)=IT_STATS.qu_tot(N_IT+1)+1;
         elseif isa(agent{cn},'larva')
             MESSAGES.atype(cn)=4;
             IT_STATS.la_tot(N_IT+1)=IT_STATS.la_tot(N_IT+1)+1;
         end
